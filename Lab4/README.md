...
Алгоритм реализован с помощью рекурсии. Начинаем от стартового значения и пытаемся рассчитать f1(...) однако для этого нам нужно знать f2(...) и так мы доходим до известного fn+1(...). После этого начинается обратный проход где уже заполняются значения предидущих f. Для каждого входного значения f сохраняются результат вычислений, что позволяет не считать одну и ту же ветвь по несколько раз.
